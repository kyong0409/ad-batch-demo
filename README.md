...
스크립트가 minikube 기준으로 작성되어 있어 별도 설정 없이 사용 가능합니다.
...
```bash
# Docker Hub에서 이미지 Pull (레포지토리 경로는 실제 업로드된 경로로 대체)
docker pull <dockerhub-user>/batch-api:latest
docker pull <dockerhub-user>/batch-ui:latest
docker pull <dockerhub-user>/ad-batch-demo:latest
docker pull <dockerhub-user>/failing-batch-demo:latest
docker pull <dockerhub-user>/enterprise-batch:latest           # 선택
docker pull <dockerhub-user>/enterprise-failing-batch:latest   # 선택

# 로컬 이미지명으로 태그 (DAG에서 로컬 이미지명을 참조하므로)
docker tag <dockerhub-user>/batch-api:latest batch-api:latest
docker tag <dockerhub-user>/batch-ui:latest batch-ui:latest
docker tag <dockerhub-user>/ad-batch-demo:latest ad-batch-demo:latest
docker tag <dockerhub-user>/failing-batch-demo:latest failing-batch-demo:latest
docker tag <dockerhub-user>/enterprise-batch:latest enterprise-batch:latest
docker tag <dockerhub-user>/enterprise-failing-batch:latest enterprise-failing-batch:latest
```
...
minikube image load ad-batch-demo:latest
minikube image load failing-batch-demo:latest
minikube image load enterprise-batch:latest           # 선택
minikube image load enterprise-failing-batch:latest   # 선택

# kind
kind load docker-image batch-api:latest batch-ui:latest ad-batch-demo:latest failing-batch-demo:latest

# k3d
k3d image import batch-api:latest batch-ui:latest ad-batch-demo:latest failing-batch-demo:latest
```
...
```bash
# Batch API
kubectl create deployment batch-api --image=batch-api:latest -n ad-batch
...
# Batch UI
kubectl create deployment batch-ui --image=batch-ui:latest -n ad-batch
...
DAG 파일 4개를 ConfigMap으로 등록합니다. 아래 DAG 파일들을 다운로드하거나 직접 작성하세요.

**ad_batch_dag.py** (성공 Job)
...
with DAG(
    ...
    default_args=default_args,
    description='Data sync batch (failure simulation)',
    schedule=None,
    catchup=False,
    is_paused_upon_creation=True,
    ...
    run_data_sync = KubernetesPodOperator(
        ...
        namespace='ad-batch',
        image='failing-batch-demo:latest',
        image_pull_policy='Never',
        env_vars={'SPRING_PROFILES_ACTIVE': 'k8s'},
        on_finish_action='delete_succeeded_pod',
        get_logs=True,
...
> 엔터프라이즈 DAG 2개(`enterprise_batch_dag.py`, `order_sync_dag.py`)는 프로젝트의  
> `05_enterprise-batch/k8s/airflow/dags/`, `06_enterprise-failing-batch/k8s/airflow/dags/`를 참고하세요.

DAG 파일을 ConfigMap으로 등록:

```bash
kubectl create configmap ad-batch-dags \
  --from-file=ad_batch_dag.py \
  --from-file=data_sync_dag.py \
  --namespace ad-batch \
